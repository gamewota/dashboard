  ...
  return (
    <div className="drawer z-50">
      ...
      <div className="drawer-side">
        <label htmlFor="my-drawer" aria-label="close sidebar" className="drawer-overlay" data-theme="bumblebee"></label>
        <div className="menu bg-base-200 text-base-content min-h-full p-4 relative z-50 flex flex-col w-80">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="font-bold text-lg">Admin</div>
            </div>
            <label htmlFor="my-drawer" className="btn btn-ghost btn-sm" aria-label="close sidebar">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <nav className="mt-4 overflow-y-auto">
            <ul className="flex flex-col gap-0 [&>li:not(:first-child)]:border-t [&>li:not(:first-child)]:border-base-300">
                    ...
                    {group.children?.filter((c: MenuItem) => isItemVisible(c, hasPerm)).map((item: MenuItem) => {
                      ...
                      return (
                        <li key={item.key} className={`px-1`}>
                          <div className={`flex items-center gap-2 rounded-md px-2 py-2 cursor-pointer ${active ? 'bg-base-100 border-l-4 border-primary' : 'hover:bg-base-200'}`}>
                            {item.icon}
                            {item.path ? (
                          ...
                          {hasChildren && openKeys[item.key] && (
                            <ul className="mt-1 ml-6">
                              {item.children!.filter((c: MenuItem) => isItemVisible(c, hasPerm)).map((child: MenuItem) => (
                                <li key={child.key} className="py-1 hover:bg-base-200">
                                  <Link to={child.path ?? '#'} className="flex items-center gap-2 px-2 rounded-md">
